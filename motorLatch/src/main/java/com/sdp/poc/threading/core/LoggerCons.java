...
import static com.sdp.poc.threading.base.msg.Color.RESET;
...
public class LoggerCons implements ILogger {
    ...
    private Color color = RESET;

    private PrintStream out = System.out;

    public LoggerCons() {
//        this.cola = Commarea.getInstance().qlog;
        this.latch = latch;
//        level = Commarea.getInstance().level;
    }
    @Override
    public void run() {
        long mark = 0L;
        try {
            while (mark >= 0 || mark < Long.MAX_VALUE) {
                ...
                String[] toks = msg.split(Commarea.TOK);
